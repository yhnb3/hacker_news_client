...
class NewsFeedApi {
  ...
  getData() : NewsFeed[] {
    return this.getReqeust<NewsFeed[]>(this.NEWS_URL);
  }
}
...
class NewsDetailApi {
  ...
  getData() : NewsDetail {
    return this.getReqeust<NewsDetail>(CONTENT_URL.replace('@id', this.id));
  }
}
...
class NewsFeedView extends View {
  ...
  render() : void {
    ...
    for (let i = (store.currentPage - 1) * 10; i < store.currentPage * 10; i++) {
      const {id, read, title,  comments_count, user, points, time_ago } = this.feeds[i]
      this.addHtml(`
    ...
    this.setTemplateData('next_page', String(store.currentPage * 10 < store.feeds.length ? store.currentPage + 1 : store.currentPage))
